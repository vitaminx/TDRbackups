...
echo 【开始转存】......
gclone copy remote:{$link} "remote:{$Copyfolderid}/$rootName" --drive-server-side-across-configs -vvP --transfers=20 --log-file=/root/AutoRclone/LOG/"$rootName"'.txt'
echo 【查缺补漏】......
gclone copy remote:{$link} "remote:{$Copyfolderid}/$rootName" --drive-server-side-across-configs -vvP --transfers=40 --log-file=/root/AutoRclone/LOG/"$rootName"'_check.txt'
echo 【去重检查】......
gclone dedupe newest "remote:{$Copyfolderid}/$rootName" --drive-server-side-across-configs -vvP --log-file=/root/AutoRclone/LOG/"$rootName"'_dedupe.txt'
echo -e
